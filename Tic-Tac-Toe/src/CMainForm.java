import lab14.CConfig;
import lab14.JGamePanel;
...
public class CMainForm extends javax.swing.JFrame implements Runnable{
    ...
    private void buttonStartClick(){
        ...
        if(CConfig.circle) setTitle("Tic Tac Toe: [O]");
            else setTitle("Tic Tac Toe: [X]");
    }
    private void buttonRestartClick(){
        ...
        try {
            dos.writeInt(999);
            dos.flush();
        }catch (IOException e1){CConfig.errors++;}
        CConfig.yourTurn = false;
        gamePanel.repaint();
    }
    ...
    public boolean checkWin(boolean myWin){
        ...
        for (int[] win : CConfig.wins){
            if(CConfig.board[win[0]] == null || CConfig.board[win[1]] == null
                    || CConfig.board[win[2]] == null) continue;
            if(CConfig.board[win[0]].equals(str) && CConfig.board[win[1]].equals(str)
                    && CConfig.board[win[2]].equals(str)){
